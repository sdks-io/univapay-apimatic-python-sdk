...
|  --- | --- | --- | --- |
| `id` | `uuid\|str` | Optional | Unique identifier for the webhook. |
| `store_id` | `uuid\|str` | Optional | ID of the store this webhook belongs to (null for merchant-level webhooks). |
| `merchant_id` | `uuid\|str` | Optional | ID of the merchant this webhook belongs to. |
| `triggers` | [`List[WebhookTrigger]`](../../doc/models/webhook-trigger.md) | Optional | List of event types that trigger this webhook. |
| `url` | `str` | Optional | The endpoint URL that receives webhook POST requests. |
| `auth_token` | `str` | Optional | Optional bearer token included in the `Authorization` header of webhook requests. Used to authenticate the webhook receiver. |
| `active` | `bool` | Optional | Whether this webhook is currently active and receiving events. |
| `is_integration` | `bool` | Optional | Admin-only flag. Indicates this webhook is used for platform integration purposes. Not settable by merchants. |
...
webhook = Webhook(
    ...
    created_on=dateutil.parser.parse('2026-04-01T00:00:00Z'),
    updated_on=dateutil.parser.parse('2026-04-02T00:00:00Z'),
