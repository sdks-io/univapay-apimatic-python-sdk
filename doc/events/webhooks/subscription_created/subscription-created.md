...
Fired when a new subscription is created and its first payment has been initiated. The `data` field contains the full Subscription object.
...
This event's request payload is of type [SubscriptionCreatedWebhookCallback](../../../../doc/models/subscription-created-webhook-callback.md).
...
def Webhooks() -> Response:
    ...
    # Step 3: Pattern match for subscriptionCreated only.
    if (
