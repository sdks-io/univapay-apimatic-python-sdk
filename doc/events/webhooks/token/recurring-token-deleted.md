...
Fired when a recurring transaction token is deleted. The `data` field contains the deleted TransactionToken object.
...
def Webhooks() -> Response:
    ...
    # Step 3: Pattern match for recurringTokenDeleted only.
    if (
