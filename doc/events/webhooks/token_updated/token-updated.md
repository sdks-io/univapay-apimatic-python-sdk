...
Fired when a transaction token is updated (e.g., metadata change). The `data` field contains the full TransactionToken object.
...
This event's request payload is of type [TokenUpdatedWebhookCallback](../../../../doc/models/token-updated-webhook-callback.md).
...
def Webhooks() -> Response:
    ...
    # Step 3: Pattern match for tokenUpdated only.
    if (
