...
class CustomsDeclarationWebhookCallback(object):
    """Implementation of the 'CustomsDeclarationWebhookCallback' model.

    Webhook envelope whose `data` payload is a CustomsDeclaration resource.

    Attributes:
        event (CustomsDeclarationEvent): Event type discriminator — always
            `customs_declaration_finished` for this callback.
        id (uuid|str): Unique ID of this webhook delivery.
        created_on (datetime): Timestamp when the event was fired.
        data (CustomsDeclarationWebhookData): Customs declaration payload delivered
            in `customs_declaration_finished` webhooks. Platform-level deliveries may
            include `platform_id` and `updated_on`.
    ...
    def __init__(
        ...
        """Initialize a CustomsDeclarationWebhookCallback instance."""
    ...
    def from_dictionary(cls,
        ...
        data =\
            CustomsDeclarationWebhookData.from_dictionary(
                dictionary.get("data"))\
