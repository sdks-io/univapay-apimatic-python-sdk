...
from univapayclientsdk.models.subscription_event import (
    SubscriptionEvent,
)


class SubscriptionWebhookEvent(object):
    """Implementation of the 'SubscriptionWebhookEvent' model.
    ...
    `subscription_failure` when a scheduled payment fails, `subscription_canceled`
    when a subscription is cancelled before all payments complete, and
    `subscription_suspended` when a subscription is paused. The `data` field contains
    the full Subscription object at the time of the event.

    Attributes:
        id (uuid|str): Unique ID of this webhook delivery.
        event (SubscriptionEvent): Event type discriminator — `subscription_created`,
            `subscription_payment`, `subscription_completed`, `subscription_failure`,
            `subscription_canceled`, or `subscription_suspended`.
        data (Subscription): The Subscription object represents a recurring payment
            schedule.
    ...
    def __init__(
        ...
        """Initialize a SubscriptionWebhookEvent instance."""
        ...
        if isinstance(dictionary, cls):
            return APIHelper.is_valid_type(
                ...
                and APIHelper.is_valid_type(
                    ...
                    type_callable=lambda value:
                        SubscriptionEvent.validate(value)) \
                and APIHelper.is_valid_type(
