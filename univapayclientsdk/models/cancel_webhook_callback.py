...
class CancelWebhookCallback(object):
    """Implementation of the 'CancelWebhookCallback' model.

    Webhook envelope whose `data` payload is a Cancel resource.

    Attributes:
        event (CancelEvent): Event type discriminator — always `cancel_finished` for
            this callback.
        id (uuid|str): Unique ID of this webhook delivery.
        created_on (datetime): Timestamp when the event was fired.
        data (Cancel): Represents a cancellation request for a charge.
    ...
    def __init__(
        ...
        """Initialize a CancelWebhookCallback instance."""
