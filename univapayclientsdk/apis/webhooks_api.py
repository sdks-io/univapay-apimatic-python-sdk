...
class WebhooksApi(BaseApi):
    ...
    def redeliver_webhook_event(self,
        ...
        Args:
            store_id (uuid|str): The unique identifier of the store.
            id (uuid|str): The unique identifier of the resource.
            event_id (uuid|str): The unique identifier of the webhook event.
            idempotency_key (str, optional): An optional idempotency key to prevent
                double charges and duplicate operations. We recommend a randomly
                generated UUID (v4).
        ...
        Returns:
            ApiResponse: An object with the response value as well as other useful
                information such as status codes and headers. Redelivery accepted.
                Returns an empty JSON object; delivery remains asynchronous.
        ...
        return super().new_api_call_builder.request(
            RequestBuilder().server(Server.DEFAULT)
            .path("/stores/{storeId}/webhooks/{id}/events/{eventId}/redeliver")
            .http_method(HttpMethodEnum.POST)
            .template_param(Parameter()
                .key("storeId")
                .value(store_id)
