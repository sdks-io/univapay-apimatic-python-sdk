"""
 univapay_client_sdk

 This file was automatically generated for Univapay by APIMATIC v3.0 (
https://www.apimatic.io ).
"""
from __future__ import annotations

from typing import Union

from univapayclientsdk.api_helper import ApiHelper
from univapayclientsdk.events.unknown_event import (
    UnknownEvent,
)
from univapayclientsdk.http.request import Request
from univapayclientsdk.models.recurring_token_deleted_webhook_callback import (
    RecurringTokenDeletedWebhookCallback,
)
from univapayclientsdk.utilities.union_type_lookup import (
    UnionTypeLookUp,
)

RecurringTokenDeletedEventType = Union[
    RecurringTokenDeletedWebhookCallback,
    UnknownEvent,
]

class RecurringTokenDeletedHandler:
    """
     Recurring token deleted event.
    """

    @staticmethod
    def parse_event(request: Request) -> RecurringTokenDeletedEventType:
        """
         Parse the event.

        :return: RecurringTokenDeletedWebhookCallback for successful parsing;
                 UnknownEvent for unknown events.
        :rtype: Union[RecurringTokenDeletedWebhookCallback, UnknownEvent]
        """
        # Deserialize payload
        try:
            union = UnionTypeLookUp.get("recurringTokenDeleted")
            return APIHelper.deserialize_union_type(union, request.raw_body)
        except Exception as e:
            return UnknownEvent([
                "Deserialization failed.",
                str(e),
            ])
