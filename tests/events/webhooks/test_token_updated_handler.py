...
class TestTokenUpdatedHandler(unittest.TestCase):
    """
     Unit tests for the `TokenUpdatedHandler` event group handler.
    """
    ...
    def _make_request(body_obj) -> Request:
        ...
        """
        raw = TestTokenUpdatedHandler._json_bytes(body_obj)
        return Request(
            method="POST",
            path="/webhooks",
    ...
    def test_token_updated_from_token_updated_handler(self):
        """
    ...
    def test_unknown_event(self):
        """
         Tests the `UnknownEvent` case from TokenUpdatedHandler.
        """
        # arrange
        event_payload = ""
        core_req = self._make_request(event_payload)
