...
class TestTokenThreeDsUpdatedHandler(unittest.TestCase):
    """
     Unit tests for the `TokenThreeDsUpdatedHandler` event group handler.
    """
    ...
    def _make_request(body_obj) -> Request:
        ...
        """
        raw = TestTokenThreeDsUpdatedHandler._json_bytes(body_obj)
        return Request(
            method="POST",
            path="/webhooks",
    ...
    def test_token_three_ds_updated_from_token_three_ds_updated_handler(self):
        """
    ...
    def test_unknown_event(self):
        """
         Tests the `UnknownEvent` case from TokenThreeDsUpdatedHandler.
        """
        # arrange
        event_payload = ""
        core_req = self._make_request(event_payload)
