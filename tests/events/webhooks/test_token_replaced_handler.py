...
class TestTokenReplacedHandler(unittest.TestCase):
    """
     Unit tests for the `TokenReplacedHandler` event group handler.
    """
    ...
    def _make_request(body_obj) -> Request:
        ...
        """
        raw = TestTokenReplacedHandler._json_bytes(body_obj)
        return Request(
            method="POST",
            path="/webhooks",
    ...
    def test_token_replaced_from_token_replaced_handler(self):
        """
    ...
    def test_unknown_event(self):
        """
         Tests the `UnknownEvent` case from TokenReplacedHandler.
        """
        # arrange
        event_payload = ""
        core_req = self._make_request(event_payload)
